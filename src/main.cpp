...
#if defined(NDEBUG)
# error "Aglehcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf51a7ffd799765e03576d6f49750a5e51719d262e2f27451624ccb24d31344f3");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Aglehcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Aglehcoin Signed Message:\n";
...
{
    // Aglehcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Aglehcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Aglehcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Aglehcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Aglehcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Aglehcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf51a7ffd799765e03576d6f49750a5e51719d262e2f27451624ccb24d31344f3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04d19ae6de5f05bceb7d978074e4e6fd5e7f70858023996766b4e309978f7b544fdc9cc8b29d060d3ef652759c2cadd20c49c11f5b1a7bf5f8449d4effc47eced6") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1610862894;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084600210;

        if (fTestNet)
        {
            block.nTime    = 1610862883;
        ...
        assert(block.hashMerkleRoot == uint256("0xc664c7bb2a62ff409c6f47919cec4d813d354481263416290ae7bc4fc3c09a57"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc7, 0xb8, 0xdb }; // Aglehcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("AglehcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("AglehcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static AglehcoinMiner(CWallet *pwallet)
{
    printf("AglehcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("aglehcoin-miner");
    ...
    try { loop {
        ...
        printf("Running AglehcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("AglehcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&AglehcoinMiner, pwallet));
}
